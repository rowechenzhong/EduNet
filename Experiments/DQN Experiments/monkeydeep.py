    ...
    for step in range(episodes):
        ...
        while not done:
            ...
            if np.random.uniform(0, 1) < eps:
                ...
                q0 = Q.feed_forward(
                    np.pad(S, (0, 1), 'constant', constant_values=0).reshape((-1, 1)))
                q1 = Q.feed_forward(
                    np.pad(S, (0, 1), 'constant', constant_values=1).reshape((-1, 1)))
                a = int(q1 > q0)

            next, reward, done, *_ = env.step(a)

            if not done:
                next0 = np.pad(next, (0, 1), 'constant',
                               constant_values=0).reshape((-1, 1))
                next1 = np.pad(next, (0, 1), 'constant',
                               constant_values=1).reshape((-1, 1))
                Sa = np.pad(S, (0, 1), 'constant',
                            constant_values=a).reshape((-1, 1))
